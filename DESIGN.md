...
### Focus — `.loupe` / `.loupe-card`

A rectangle dragged over a **stopped** frame, redrawn with an unsharp mask: `original + amount × (original − blurred)`.

Its job is the softness the *zoom* introduces. At 300% the window is stretching one video pixel over nine, and what the eye reads is mush that belongs to the resampler, not to the encoder. The tool gives that edge back so the block underneath can be judged on its own terms.

It invents nothing. It raises the contrast already present at an edge, which is what the eye reads as sharpness — so it can sit beside a judgement about compression without falsifying it, *provided* the user remembers what it does. A strong amount also puts an edge on the side of a macroblock, and a block with a crisp edge looks like detail. That is what **Comparar** is for, and why it is in the card rather than buried: an enhancement with nothing to compare against always looks like an improvement, because nothing on screen says how much it added.

- **Armed, not always on.** The `Enfocar` key in the transport is a toggle (`aria-pressed`, `.tkey.is-on` — the Neutral Selection Rule again), `disabled` while the video runs. While armed the table's cursor is `crosshair` and a drag draws instead of pans; a rectangle under 2% of the frame is treated as a click and discarded. Selecting disarms the tool, so panning comes straight back.
- **The rectangle is stored in frame proportions (0–1), never in screen pixels**, and the canvas is positioned inside `.stage-frame` in percentages. Zoom, pan and window resize move it for free, and the same rectangle keeps meaning the same pixels.
- **It is redrawn on every new frame while paused**, so stepping with `J`/`K` walks the focused region forward one frame at a time — the single most useful thing it does, because the frame after a cut is the worst one in the video.
- **Sampled at the scale it is being *seen*, not the scale it is stored at.** The canvas is sized `source × fit × zoom × devicePixelRatio` (capped at 4× and at 4M pixels), the region is drawn into it with the engine's best filter, and the mask runs on those already-stretched pixels with the radius stretched by the same factor. This is the whole point: an unsharp mask applied *before* the browser's upscale is dissolved by it, and a radius left in source pixels would bite a finer and finer detail as you zoom in, never touching the edge the zoom actually smeared. The radius control stays in file pixels, because that is the unit the user is judging in.
- **Blurred with `ceil(radius × 3)` pixels of margin** that are then cropped off: blurring exactly the rectangle would bleed its edges into the transparent nothing outside the canvas and ring the region with a dark halo.
- **Re-rendered inside a `requestAnimationFrame`.** Zoom is now an input to the result, and a wheel spun fast would otherwise chain twenty full mask passes nobody will see. Only the last survives.
- **No `image-rendering: pixelated`.** The canvas now carries display resolution, so there is normally nothing left to resample; when there is (the 4× / 4M caps), it must interpolate exactly like the video underneath it, because the whole point is comparing the two — a different resampling rule on either side of the hairline would be the comparison lying.
- **`.loupe-card`** is a raised paper card on the table (bottom-left, `--lift-stage`): amount, radius, **Comparar**, **Quitar**. Text is never set on the table itself. The rectangle's hairline is `--sheet` — paper, not foil: focusing a region commits no work, and the Single Foil Rule holds.
...
- **Capture saves what is on the table.** The visible rectangle is derived from the same pan and zoom that transform the frame, so it is not an estimate of what is on screen — it *is* what is on screen. Zoomed in, the file is that crop; fitted, it is the whole frame. **It crops but never stretches:** pixels are written at file resolution, where a macroblock measures what it measures — saving the zoom would weigh nine times more without adding one pixel of information. The visible enhancement is composited in, because turning it off at write time would hand back a different image than the one that made the user press the button (**Comparar** turns it off on the table, and then it does not go in either).
- **The button says which of the two it will do.** `Capturar fotograma` when the whole frame is visible, `Capturar lo visible` when it is not, with the exact pixel size in the `title`. Promising "frame" and writing a crop would be lying on the one button in the module that touches the disk. The name follows: `-cuadro-N-recorte.png`, because a crop and the full frame of the same frame number are not the same file and whoever goes looking later needs to tell them apart.
