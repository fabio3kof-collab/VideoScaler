...
A rectangle dragged over a **stopped** frame, redrawn from the source pixels with an unsharp mask: `original + amount × (original − blurred)`.
...
- **Sampled at source resolution**, and blurred with `ceil(radius × 3)` pixels of margin that are then cropped off: blurring exactly the rectangle would bleed its edges into the transparent nothing outside the canvas and ring the region with a dark halo.
- **No `image-rendering: pixelated`.** The overlay is interpolated exactly like the video underneath it, because the whole point is comparing the two — a different resampling rule on either side of the hairline would be the comparison lying.
